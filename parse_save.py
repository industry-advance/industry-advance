...
def map_file_to_map(path: str) -> Tuple[int, int, str, str]:
    ...
    with open(path, "rb") as f:
        ...
        if not os.path.exists("/tmp/msav-converter"):
            os.mkdir("/tmp/msav-converter")
        # TODO: Check if writeable... 2 different users on the same machine can't build the assets ATM
